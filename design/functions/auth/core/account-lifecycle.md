...
- Block new login, authorization-code issuance, token refresh, and userinfo responses.
- Preserve identity mappings and contact metadata so the account can be reviewed or re-enabled.

## Enable User

`enable_user(subject)` reverses only a disabled account.

It should:

- Allow `disabled -> active`.
- Treat `active -> active` as idempotent.
...
- Block new login, authorization-code issuance, token refresh, and userinfo responses for that subject.
