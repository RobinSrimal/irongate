...
Access tokens are self-contained JWTs. The token endpoint issues signed tokens and persists only the
refresh-token state needed for rotation and logout.
...
- Access-token validation is local JWT validation by resource APIs.
