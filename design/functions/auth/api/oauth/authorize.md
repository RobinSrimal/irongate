...
The endpoint supports authorization-code flow plus OIDC `scope=openid` and optional client `nonce`.

Requests that include unsupported parameters that change authentication semantics fail safely instead
of silently pretending to honor them.

Because the target core is API-only, applications provide login, consent, account-selection, and
provider-selection UI in front of these endpoints.
...
- The endpoint returns protocol responses and redirects only.
