...
OAuth clients are config-only in v1, so there is no `client_config(client_id)` DynamoDB key in the target runtime table. If runtime client management is added later, it needs a separate control-plane design.
