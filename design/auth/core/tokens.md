...
User-facing logout revokes refresh-token state through `POST /oauth/revoke`. It stops future refreshes for that session but does not invalidate already-issued access JWTs.
...
V1 access tokens are self-contained JWTs. Resource APIs validate them locally using:
...
There is no token introspection endpoint in v1, and access tokens are not stored for server-side lookup.
...
- Resource APIs validate access tokens locally; the auth server does not provide access-token introspection in v1.
